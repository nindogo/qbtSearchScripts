...
class yourbittorrent(object):
    ...
    GET_SIZE_RE = (r'</b></td></tr></thead><tr><td width=120px><b>Size</td><td>(.+) in [\d*] ') # Not the best regex
    ...
    def find_results(self, desc_link):
        ...
        # except ConnectionError: quit()

        this_result['engine_url'] = URL
        this_result['name'] = str(re.findall(self.GET_NAME_RE, results_page)[0]).replace('|', '').replace('[', '').replace(']', '')
        ...
        # print('desc_link',this_result['desc_link'])
        # print('name', this_result['name'])
        # print('seeds', this_result['seeds'])
        # print('leech', this_result['leech'])
        # print('size', this_result['size'])
